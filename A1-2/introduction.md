...
아래 중 택1로 구현한다. 국내 장소 검색이 가능해야 한다.
...
- Kakao Local 예시: 인증 헤더에 키를 실어 요청하는 방식
  - 401/403 발생 시 키/권한/도메인 설정을 점검
- Naver Local Search 예시: 클라이언트 ID/Secret을 헤더로 보내는 방식
  - 401/403 발생 시 키 값/헤더명 오타를 점검
...
- 지도/장소 API 실패(네트워크/인증/쿼터 등):
  - 맛집 섹션을 “데이터 없음” 처리
  - 리포트 생성은 계속 진행
- LLM JSON 파싱 실패:
  - 재요청 1회 또는
  - “필수 키만 다시 JSON으로 출력”하도록 프롬프트를 수정해 재시도 1회

실패가 발생하면, 결과 리포트에 `errors` 섹션으로 요약을 남길 수 있도록 내부적으로 오류 목록을 관리한다. 빈 리스트여도 무방하다.
...
- 1차 추천에서 `recommended_city`를 1개가 아니라 2~3개로 확장한다.
  - 예: `recommended_cities: [..]`
...
### 8.4 지도/장소 API 인증 실패(401/403) 케이스
