...
def build_parser():
    ...
    p = sub.add_parser("import", parents=[common],
                       help="CSV 수집 -> 정제 -> DB 저장 (B 모듈 사용)")
    p.add_argument("--file", "-f", required=True, help="CSV 파일 경로")
    ...
    # ---------------- stats
    p = sub.add_parser("stats", parents=[common], help="통계 요약")
...
def cmd_import(args, config):
    """
    CSV -> (B) raw JSONL -> DB raw 저장 -> (B) 정제 -> DB clean 저장

    [경계]
      B: 파일 읽기 / 검증 / 정제  (source/src/importer.py, cleaner.py)
      A: raw 보관, 해시 계산, 중복 정책, DB 저장
    """
